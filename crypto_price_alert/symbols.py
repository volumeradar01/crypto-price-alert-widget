...
# ----- stocks: live search, not a pre-cached list (the universe is huge) -----

_STOCK_HEADERS = {"Accept": "application/json", "User-Agent": "CryptoPriceAlert/1.1"}


def search_stocks(query: str, market: str = "") -> list[tuple[str, str, str]]:
    """Live Yahoo Finance ticker search (blocking). Returns up to 12
    ``(symbol, name, exchange_display)`` tuples, equities only. ``market`` is
    ``"US"`` or ``"IN"`` and is used only to rank matching results first."""
    ...
    for q in rows:
        ...
        out.append((symbol, name, exch))

    def _is_indian(sym: str) -> bool:
        return sym.upper().endswith((".NS", ".BO"))

    if market == "IN":
        out.sort(key=lambda t: 0 if _is_indian(t[0]) else 1)
    elif market == "US":
        out.sort(key=lambda t: 1 if _is_indian(t[0]) else 0)
    return out[:12]
